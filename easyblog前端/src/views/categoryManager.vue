  ...
  <div class="page">
    <el-container>
      ...
      <el-header class="managerAllHeaderView">
        <div class="leftView">
          <el-popconfirm title="您确定要删除这个博客吗？" @confirm="handleDelete(null)">
            <template #reference>
        ...
        <div class="rightView">
          <el-button type="danger" v-if="currentUser.username == 'admin'" @click="TurnToAllManager()"> 管理 </el-button>
          <el-button type="danger" @click="TurnToDraft()"> 草稿箱 </el-button>
      ...
      <el-main class="el-main">
        <el-table ref="multipleTableRef" id="categoryTable" stripe :data="categoryVoList" style="width: 100%" class="categoryModel" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="categorySelf" label="查看权限" width="200" min-width="200" align="center">
            <template v-slot="{ row }">
              <el-switch v-model="row.categorySelf" v-if="row.userId == userid" @change="categorySelfChangeHandle(row)" active-text="共享" inactive-text="仅自己可见" active-value="0" inactive-value="1">
              </el-switch>
              <div v-else class="categorySelfShare">
                <img style="width: 50%; height: 100%" src="../assets/share.png" />
              </div>
            </template>
          </el-table-column>
            ...
            <template v-slot="scope">
              <div class="slotBox">
                ...
                <div class="slotBtnBoxFour" v-if="scope.row.userId == userid">
                  <el-popconfirm title="您确定要删除这些博客吗？" @confirm="handleDelete(scope.row.categoryId)">
                    <template #reference>
...
const getUserCategoryList = async () => {
  ...
  let result = await requestUtil.get("BlogSort/userCategoryList/" + userid.value);
  if (result.data.code == 200) {
    categoryVoList.value = result.data.categoryVoList;
  }
};
...
/**跳转到管理界面 */
const TurnToAllManager = () => {
  store.commit("SET_MANAGERTYPE", 1);
  router.push("/allManager");
};
...
function setSort() {
  ...
  new Sortable(el, {
    ...
    onEnd: async (e) => {
      ...
      // 更新sort属性
      categoryVoList.value = categoryVoList.value.map((item, index) => ({ ...item, sort: index, }));
      let result = await requestUtil.post("BlogSort/updateManagerVoList", categoryVoList.value);
